...
- **Dynamic 3D / Reality**: `<Reality>` component for building 3D scenes with primitives (`BoxEntity`, `SphereEntity`, etc.), materials (`UnlitMaterial`), and model instances (`ModelAsset` + `ModelEntity`). Use when the user needs procedural 3D content or animated primitives.
